...
import { authenticateAccess, createProfile, getProfiles, migrateExistingData, type Profile } from '@/lib/api/users'
...
export function UserSelectScreen({ onSelect }: Props) {
  ...
  async function handleLogin() {
    ...
    try {
      await authenticateAccess(password)
          ...
          <div className="rounded-[28px] bg-[#18202a] p-[24px] shadow-[0px_16px_50px_0px_rgba(25,28,30,0.12)]">
            ...
            <div className="flex flex-col gap-[10px]">
              ...
              <button
                ...
                className={`${FONT} mt-[6px] rounded-[14px] py-[14px] text-[15px] font-semibold text-white transition-colors ${selectedProfile ? 'bg-[#004ea7] hover:bg-[#0b5cbb]' : 'cursor-not-allowed bg-[#33445a] text-[#8ea1ba]'}`}
              >
                입장하기
              </button>
              ...
              {profiles.length > 0 && (
                <p className={`${FONT} text-[12px] text-[#8ea1ba]`}>
                  비밀번호는 서버에서 확인되며, 확인 후 데이터 접근 세션이 이 기기에 저장됩니다.
                </p>
              )}
            </div>
          </div>
